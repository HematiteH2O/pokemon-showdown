...
exports.BattleStatuses = {
	blst: {
		...
		onStart(target, source, sourceEffect) {
			if (sourceEffect && sourceEffect.id === 'flameorb') {
				this.add('-status', target, 'blst', '[from] item: Flame Orb');
			} else if (sourceEffect && sourceEffect.effectType === 'Ability') {
				this.add('-status', target, 'blst', '[from] ability: ' + sourceEffect.name, '[of] ' + source);
			} else {
				this.add('-status', target, 'blst');
			}
		},
		onBasePowerPriority: 19,
		onBasePower(basePower, attacker, defender, move) {
			if (!attacker.hasAbility('guts') && move.id !== 'facade' && move.category === 'Physical') {
	...
	psn: {
		...
		onStart(target, source, sourceEffect) {
			if (source.hasType('Poison')) {
				this.add('-message', target.name + " was envenomed!");
			}
			if (sourceEffect && sourceEffect.effectType === 'Ability') {
		...
		onModifyDef(def, pokemon) {
			if (this.effectData.source.hasType('Poison') && !pokemon.hasAbility('marvelscale')) {
				return this.chainModify([0x0AAA, 0x1000]);
			}
		},
		onResidualOrder: 9,
	...
	bane: {
		...
		onStart(target, source, sourceEffect) {
			if (sourceEffect && sourceEffect.effectType === 'Ability') {
				this.add('-status', target, 'bane', '[from] ability: ' + sourceEffect.name, '[of] ' + source);
			} else {
				this.add('-status', target, 'bane');
			}
		},
		onModifyDef(def, pokemon) {
	...
	hail: {
		...
		onWeatherModifyDamage(damage, attacker, defender, move) {
			if (defender.hasType('Ice') && defender.getMoveHitData(move).typeMod > 0) {
				this.debug('hail Filter neutralize');
